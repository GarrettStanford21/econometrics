...
collapse_function_pay = function( i ){
  pay_df_i = asec_df %>% 
    filter(YEAR == i , INCWAGE < 9999998 ) %>% 
    group_by(STATEFIP , YEAR ) %>% 
    summarise( pay_avg = weighted.mean(INCWAGE , w = ASECWT) )
}
collapse_function_hrs = function( i ){
  hrs_df_i = asec_df %>% filter(YEAR == i , UHRSWORKLY < 999 ) %>% 
    group_by(STATEFIP , YEAR ) %>% 
    summarise( hrs_avg = weighted.mean(UHRSWORKLY , w = ASECWT) )
  }
collapse_function_lf1 = function(i){
  asec_lf1_i = asec_df %>% filter(AGE>13 & YEAR==i) %>% 
    group_by( STATEFIP , YEAR ) %>% 
    summarise( ilf = weighted.mean(LABFORCE==2 , w = ASECWT ),
               unemployment = weighted.mean(EMPSTAT==20|EMPSTAT==21|EMPSTAT==22 , w = ASECWT )
    )
}
collapse_function_lf = function(i){
  asec_lf_i = asec_df %>% filter(AGE>14 & YEAR==i) %>% 
    group_by( STATEFIP , YEAR ) %>% 
    summarise( ilf = weighted.mean(LABFORCE==2 , w = ASECWT ),
               unemployment = weighted.mean(EMPSTAT==20|EMPSTAT==21|EMPSTAT==22 , w = ASECWT )
    ) 
...
collapse_function_vet1 = function(i){
  asec_vet1_i = asec_df %>% filter(AGE>14 & YEAR==i) %>% 
    group_by( STATEFIP , YEAR ) %>% 
    summarise( vet = weighted.mean( VETSTAT==2 , w = ASECWT ))
  
}
collapse_function_vet = function(i){
  asec_vet_i = asec_df %>% filter(AGE>16 & YEAR==i) %>% 
    group_by( STATEFIP , YEAR ) %>% 
    summarise( vet = weighted.mean( VETSTAT==2 , w = ASECWT ))
  
}
collapse_function_care = function(i){
  asec_care_i = asec_df %>% filter(AGE>14 & YEAR==i) %>% 
    group_by( STATEFIP , YEAR ) %>% 
    summarise( medicare = weighted.mean(HIMCARE==2 , w = ASECWT ) )
